...
from sam_submission.models import Paper,PCM_Review,PCM_Requests,ReviewAssignments,Submission
...
def pcc_view_submissions(request):
    submissions = Submission.objects.filter(isReviewed='0')
    requested_subs=PCM_Requests.objects.all()
    paper = Paper.objects.filter(is_revised='0').exclude(submission_id__in=[i.submission_id for i in requested_subs]).exclude(submission_id__reviewers_count__gte=3)
    args ={}
    args['paper']=paper
    return render(request,"pcc_other_papers.html",args)
